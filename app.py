...
/* CHAT HEADER */
.chat-header{height:60px;background:var(--pn);display:flex;align-items:center;gap:4px;padding:0 6px 0 4px;border-bottom:1px solid var(--bd);flex-shrink:0;}
...
<div class="sb" id="sidebar">
  ...
  <div class="sb-panel" id="panel-live">
    <div style="padding:12px 14px;">
      <button onclick="startLiveStream()" style="width:100%;padding:12px;background:linear-gradient(135deg,#e53e3e,#c53030);color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:900;cursor:pointer;">🔴 Mulai Live Streaming</button>
    </div>
    <div class="status-section-label">Live Sekarang</div>
    <div id="live-list"><div style="padding:24px;text-align:center;color:var(--st);font-size:13px;">Tidak ada siaran live 📺</div></div>
  </div>
  ...
  <div id="chat-wrap" style="display:none;flex-direction:column;height:100%;overflow:hidden;">
    <div class="chat-header" id="chat-header">
      ...
      <div onclick="showContactInfo()" style="flex:1;min-width:0;max-width:calc(100% - 140px);overflow:hidden;cursor:pointer;padding:0 4px;">
        <div id="header-name" style="font-weight:900;font-size:15px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">—</div>
    ...
    <div id="chat-more-menu" style="display:none;position:fixed;right:8px;top:62px;background:#202c33;border:1px solid #2a3942;border-radius:8px;box-shadow:0 4px 20px rgba(0,0,0,.5);z-index:999;min-width:160px;overflow:hidden;">
      <div onclick="openAI();this.parentElement.style.display='none'" style="padding:13px 18px;cursor:pointer;font-size:14px;color:#e9edef;" onmouseover="this.style.background='#2a3942'" onmouseout="this.style.background=''">🤖 Tanya AI</div>
      <div onclick="searchInChat();this.parentElement.style.display='none'" style="padding:13px 18px;cursor:pointer;font-size:14px;color:#e9edef;" onmouseover="this.style.background='#2a3942'" onmouseout="this.style.background=''">🔍 Cari Pesan</div>
    </div>
    ...
    <div class="input-area">
      ...
      <div class="input-row">
        ...
        <button class="side-btn" onclick="toggleEmojiPicker()">😊</button>
...
<div class="ai-drawer" id="ai-drawer">
  <div class="ai-drawer-header">
    <div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#9b5de5);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">🤖</div>
    ...
    <button style="background:rgba(255,255,255,.08);border:none;color:rgba(224,212,255,.6);width:30px;height:30px;border-radius:50%;font-size:16px;cursor:pointer;" onclick="closeAI()">×</button>
  </div>
  <div class="ai-messages" id="ai-messages">
    <div class="ai-msg ai"><div class="ai-msg-av">🤖</div><div class="ai-bubble">Halo! Saya <b>WaClone AI</b> 👋<br><br>Saya bisa bantu menulis pesan, menjawab pertanyaan, atau menerjemahkan teks.<br><br>Ada yang bisa saya bantu?</div></div>
  </div>
  <div class="ai-chips" id="ai-chips">
    <div class="ai-chip" onclick="sendAIMsg('Bantu saya menulis pesan yang baik')">✏️ Tulis pesan</div>
    <div class="ai-chip" onclick="sendAIMsg('Buat lelucon lucu untuk teman')">😂 Lelucon</div>
    <div class="ai-chip" onclick="sendAIMsg('Terjemahkan ke Inggris: Halo, apa kabar?')">🌐 Terjemah</div>
  </div>
...
<div class="overlay" id="prof-ov">
  <div class="panel">
    <div class="panel-header"><h2>👤 Profil Saya</h2><button class="close-btn" onclick="closePanel('prof-ov')">×</button></div>
...
<div class="overlay" id="settings-ov">
  <div class="panel" style="width:440px;">
    <div class="panel-header"><h2>⚙️ Pengaturan</h2><button class="close-btn" onclick="closePanel('settings-ov')">×</button></div>
    <div class="panel-body">
      <div style="margin-bottom:18px;"><div style="font-size:11px;font-weight:800;color:var(--g);text-transform:uppercase;letter-spacing:.6px;margin-bottom:10px;padding-bottom:5px;border-bottom:1px solid var(--bd);">💬 Chat</div>
      ...
      <div><div style="font-size:11px;font-weight:800;color:var(--g);text-transform:uppercase;letter-spacing:.6px;margin-bottom:10px;padding-bottom:5px;border-bottom:1px solid var(--bd);">🗑️ Data</div>
        <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;">
          <div><div style="font-size:13px;font-weight:700;">Hapus Cache Lokal</div></div>
          <button onclick="clearLocalCache()" style="padding:7px 14px;background:var(--dk);border:1.5px solid var(--bd);border-radius:8px;color:var(--tx);font-size:12px;font-weight:700;cursor:pointer;">🗑️ Hapus</button>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;">
          <div><div style="font-size:13px;font-weight:700;">Versi</div></div>
          <span style="font-size:12px;color:var(--g);font-weight:700;">v4.0 • Cloudinary + Supabase</span>
        </div>
      </div>
    </div>
  </div>
...
<div class="overlay" id="notif-ov">
  <div class="panel">
    <div class="panel-header"><h2>🔔 Notifikasi</h2><button class="close-btn" onclick="closePanel('notif-ov');markNotifsRead()">×</button></div>
...
<div class="overlay" id="stview-ov">
  <div class="stv" id="stv-wrap">
    ...
    <div class="stv-head">
      ...
      <button class="close-btn" onclick="closeStatusViewer()" style="background:rgba(255,255,255,.1);color:rgba(255,255,255,.7);">×</button>
    </div>
...
<div class="overlay" id="cstat-ov">
  <div class="panel">
    <div class="panel-header"><h2>✏️ Buat Status</h2><button class="close-btn" onclick="closePanel('cstat-ov')">×</button></div>
    <div class="panel-body">
      <div style="display:flex;flex-direction:column;gap:8px;">
        <div onclick="showTextStatusForm()" style="display:flex;align-items:center;gap:12px;padding:11px;background:var(--dk);border-radius:10px;border:1.5px solid var(--bd);cursor:pointer;">
          <div style="width:42px;height:42px;border-radius:50%;background:#2563eb22;display:flex;align-items:center;justify-content:center;font-size:19px;">✏️</div>
        ...
        <div onclick="document.getElementById('stat-photo-in').click()" style="display:flex;align-items:center;gap:12px;padding:11px;background:var(--dk);border-radius:10px;border:1.5px solid var(--bd);cursor:pointer;">
          <div style="width:42px;height:42px;border-radius:50%;background:#dc262622;display:flex;align-items:center;justify-content:center;font-size:19px;">🖼️</div>
        ...
        <div onclick="document.getElementById('stat-video-in').click()" style="display:flex;align-items:center;gap:12px;padding:11px;background:var(--dk);border-radius:10px;border:1.5px solid var(--bd);cursor:pointer;">
          <div style="width:42px;height:42px;border-radius:50%;background:#7c3aed22;display:flex;align-items:center;justify-content:center;font-size:19px;">🎥</div>
...
<div class="overlay" id="cam-ov">
  <div class="panel" style="width:420px;">
    <div class="panel-header"><h2>📷 Kamera</h2><button class="close-btn" onclick="closeCamera()">×</button></div>
    <div class="panel-body">
      ...
      <div class="cam-controls">
        ...
        <button class="cam-btn" style="background:var(--rd);" onclick="closeCamera()">×</button>
      </div>
...
<div class="overlay" id="fw-ov">
  <div class="panel">
    <div class="panel-header"><h2>↪️ Teruskan Pesan</h2><button class="close-btn" onclick="closePanel('fw-ov')">×</button></div>
    <div class="panel-body">
...
<div class="incoming-call" id="incoming-call">
  ...
  <div style="text-align:center;font-size:12px;color:var(--st);margin-top:4px;" id="inc-type">📞 Panggilan Masuk</div>
  <div class="inc-actions">
    <button class="inc-btn inc-accept" onclick="answerCall('audio')">📞 Audio</button>
...
<div class="live-ui" id="live-ui">
  <div class="live-header">
    <div style="display:flex;align-items:center;gap:10px;">
      ...
      <div style="background:rgba(0,0,0,.6);color:#fff;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:700;" id="live-viewer-count">👁 0 penonton</div>
    </div>
    <div style="display:flex;align-items:center;gap:8px;">
      <span id="live-streamer-name" style="color:#fff;font-size:14px;font-weight:700;"></span>
      <button onclick="closeLiveUI()" style="background:rgba(255,255,255,.15);border:none;color:#fff;width:32px;height:32px;border-radius:50%;font-size:18px;cursor:pointer;">×</button>
    </div>
  </div>
  ...
  <div class="live-controls-bottom">
    ...
    <button id="live-end-btn" onclick="endLiveStream()" style="display:none;padding:9px 18px;background:#e53e3e;color:#fff;border:none;border-radius:20px;font-size:13px;font-weight:800;cursor:pointer;">⏹ Akhiri</button>
  </div>
</div>
...
function clearLocalCache(){try{const s=localStorage.getItem('wc_settings');localStorage.clear();if(s)localStorage.setItem('wc_settings',s);toast('Cache dibersihkan ✅');}catch(e){}}
...
function renderChatList(){
  ...
  if(!others.length){el.innerHTML='<div style="padding:28px;text-align:center;color:var(--st);">Belum ada pengguna 👥</div>';return;}
...
function showContactInfo(){
  ...
  toast((u?.username||currentFriend.name)+' — '+(u?.bio||'No bio'),3000);
}
...
async function loadMessages(){
  ...
  try{
    ...
    if(f){const st=document.getElementById('header-status');st.textContent=f.online?'🟢 Online':(f.last_seen?`Terakhir ${fmtTime(f.last_seen)}`:'⚫ Offline');}
  }catch(e){}
}
function renderMessages(msgs){
  const area=document.getElementById('messages-area');
  const atBottom=area.scrollHeight-area.clientHeight<=area.scrollTop+120;
  if(!msgs.length){area.innerHTML='<div style="text-align:center;color:var(--st);padding:36px;font-size:13px;">Mulai percakapan! 👋</div>';return;}
  let html='',lastDate='';
  msgs.forEach(m=>{
    ...
    if(m.file_url){
      ...
      else{const fn=m.file_url.split('/').pop().split('?')[0].substring(0,30);content+=`<a class="file-link" href="${m.file_url}" target="_blank">📄 ${escHtml(fn)}</a>`;}
    }
    ...
    const acts=`<div class="msg-actions">
      <button class="act-btn" onclick='setReply("${m.id}","${safeTxt}","${m.from_uid}")'>↩</button>
      <button class="act-btn" onclick='showCtxMenu(event,"${m.id}","${safeTxt}","${m.from_uid}")'>⋯</button>
    </div>`;
...
async function sendCamPhoto(){
  ...
  if(camMode==='status'){fd.append('type','image');const r=await fetch('/api/status/upload',{method:'POST',body:fd});const d=await r.json();done();if(d.ok){toast('Status diposting ✅');loadSidebarStatuses();setTimeout(openMyStatus,600);}else toast('Gagal',2500,true);}
...
async function loadSidebarStatuses(){
  try{
    ...
    document.getElementById('my-status-hint').textContent=myStats.length?`${myStats.length} status · ${fmtTime(myStats[myStats.length-1].created_at)}`:'Ketuk untuk buat status';
    ...
    fl.innerHTML=entries.map(([uid,sts])=>{
      ...
      return`<div class="status-friend-item" onclick="viewFriendStatus('${uid}')">
        ...
        <div style="flex:1;min-width:0;margin-left:12px;"><div style="font-weight:800;font-size:14px;">${escHtml(u.username)}</div><div style="font-size:12px;color:var(--st);margin-top:1px;">${fmtTime(latest.created_at)} · ${sts.length} status</div></div>
      </div>`;
    }).join('');
  }catch(e){}
}
...
async function postTextStatus(){
  ...
  const d=await r.json();if(d.ok){toast('Status diposting ✅');closePanel('cstat-ov');document.getElementById('stat-text-inp').value='';document.getElementById('text-stat-form').style.display='none';await loadSidebarStatuses();setTimeout(openMyStatus,600);}else toast('Gagal: '+(d.msg||''),3000,true);
}
...
async function saveProfile(){
  ...
  if(d.ok){document.getElementById('pname').textContent=u;toast('Profil disimpan ✅');closePanel('prof-ov');setTimeout(()=>location.reload(),800);}else toast('Gagal: '+(d.msg||''),3000,true);
}
...
async function joinAgoraChannel(channelName,type){
  try{
    ...
    agoraClient.on('user-unpublished',(user,mediaType)=>{if(mediaType==='video'){document.getElementById('agora-remote').innerHTML='<div style="text-align:center;color:rgba(255,255,255,.4);"><div style="font-size:48px;margin-bottom:8px;">👤</div>Kamera dimatikan</div>';}});
...
async function endCall(){
  ...
  document.getElementById('agora-remote').innerHTML='<div style="text-align:center;color:rgba(255,255,255,.3);font-size:14px;"><div style="font-size:48px;margin-bottom:8px;">👤</div>Menunggu video...</div>';
  document.getElementById('agora-local').innerHTML='';
}

function toggleMic(){micMuted=!micMuted;if(agoraLocalAudioTrack)agoraLocalAudioTrack.setEnabled(!micMuted);const b=document.getElementById('ccbtn-mic');b.className='ccbtn btn-toggle '+(micMuted?'off':'on');toast(micMuted?'Mic dimatikan 🔇':'Mic aktif 🎤',1500);}
function toggleCam(){camOff=!camOff;if(agoraLocalVideoTrack)agoraLocalVideoTrack.setEnabled(!camOff);const b=document.getElementById('ccbtn-cam');b.className='ccbtn btn-toggle '+(camOff?'off':'on');toast(camOff?'Kamera dimatikan 🚫':'Kamera aktif 📷',1500);}
function toggleSpeaker(){speakerOff=!speakerOff;const b=document.getElementById('ccbtn-spk');b.className='ccbtn btn-toggle '+(speakerOff?'off':'on');toast(speakerOff?'Speaker dimatikan 🔇':'Speaker aktif 🔊',1500);}
...
async function startLiveStream(){
  ...
  try{
    ...
    liveCommentPollTimer=setInterval(pollLiveComments,3000);toast('Live streaming dimulai! 🔴',2000);
...
async function loadLiveList(){
  try{const r=await fetch('/api/live/list');const d=await r.json();const el=document.getElementById('live-list');const lives=d.lives||[];if(!lives.length){el.innerHTML='<div style="padding:24px;text-align:center;color:var(--st);font-size:13px;">Tidak ada siaran live saat ini 📺</div>';return;}
    el.innerHTML=lives.map(l=>`<div class="live-item" onclick="watchLive('${l.channel}','${escHtml(l.host_name||'?')}','${l.id}')">
      <div style="width:70px;height:44px;border-radius:8px;background:#1a0a38;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;position:relative;">📺<span style="position:absolute;top:3px;left:3px;background:#e53e3e;color:#fff;font-size:9px;font-weight:900;padding:1px 5px;border-radius:4px;">LIVE</span></div>
      <div style="flex:1;min-width:0;"><div style="font-weight:800;font-size:14px;">${escHtml(l.host_name||'?')}</div><div style="font-size:12px;color:var(--g);">👁 ${l.viewer_count||0} penonton</div></div>
...
async function sendAIMsg(text){
  ...
  const uDiv=document.createElement('div');uDiv.className='ai-msg user';uDiv.innerHTML=`<div class="ai-msg-av">👤</div><div class="ai-bubble">${escHtml(text)}</div>`;container.appendChild(uDiv);
  aiHistory.push({role:'user',content:text});
  const thinkId='ai-t-'+Date.now();const thinkDiv=document.createElement('div');thinkDiv.className='ai-msg ai';thinkDiv.id=thinkId;thinkDiv.innerHTML=`<div class="ai-msg-av">🤖</div><div class="ai-thinking-bubble"><span></span><span></span><span></span></div>`;container.appendChild(thinkDiv);container.scrollTop=container.scrollHeight;
  try{
    ...
    aDiv.innerHTML=`<div class="ai-msg-av">🤖</div><div class="ai-bubble">${formatted}</div>`;container.appendChild(aDiv);container.scrollTop=container.scrollHeight;
  }catch(e){const thinkEl=document.getElementById(thinkId);if(thinkEl)thinkEl.remove();const errDiv=document.createElement('div');errDiv.className='ai-msg ai';errDiv.innerHTML=`<div class="ai-msg-av">🤖</div><div class="ai-bubble" style="background:rgba(220,38,38,.1);border-color:rgba(220,38,38,.3);color:#fca5a5;">Terjadi kesalahan 😔</div>`;container.appendChild(errDiv);container.scrollTop=container.scrollHeight;}
}
