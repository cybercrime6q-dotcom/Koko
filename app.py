...
function switchTab(tab){
  ['chats','status','live','contacts'].forEach(t=>{
    const _tel=document.getElementById('tab-'+t);if(_tel)_tel.classList.toggle('active',t===tab);
    const _pel=document.getElementById('panel-'+t);if(_pel)_pel.classList.toggle('active',t===tab);
  });
...
async function loadUsers(){
  try{
    ...
    if(d.error&&!(d.users&&d.users.length)){
      document.getElementById('chat-list').innerHTML=`<div style="padding:28px;text-align:center;color:#f87171;font-size:13px;">Error: ${d.error}<br><small>Cek <a href="/api/test_db" target="_blank" style="color:#60a5fa;">/api/test_db</a></small></div>`;
      return;
    }
    allUsers=d.users||[];
    renderChatList();
    ...
    const msg=e.name==='AbortError'?'Timeout - server lambat':'Gagal terhubung';
    document.getElementById('chat-list').innerHTML=`<div style="padding:28px;text-align:center;color:#f87171;font-size:13px;">${msg}. <a href="javascript:loadUsers()" style="color:#60a5fa;">Coba lagi</a></div>`;
...
function showContactInfo(){
  ...
  toast(((u&&u.username?u.username:currentFriend.name))+' â€” '+((u&&u.bio?u.bio:'No bio')),3000);
}
...
async function loadNotifications(){
  ...
  el.innerHTML=notifs.slice().reverse().map(n=>{const u=allUsers.find(x=>x.uid===n.from_uid);const nm=(u&&u.username)?u.username:'?';
    return`<div class="notif-item" onclick="closePanel('notif-ov');openChat('${n.from_uid}','${(nm).replace(/'/g,"\\'")}','${(u&&u.avatar?u.avatar:'')}','')">
...
async function startCall(type){
  ...
  if(typeof AgoraRTC==='undefined'){toast('Agora SDK belum siap, coba lagi.',2500,true);return;}
...
// Start immediately - don't wait for Agora
...
def api_users():
    ...
    try:
        ...
        if not isinstance(all_users, list):
            return jsonify({"users":[], "error":"Gagal ambil data users dari Supabase"})
        users = []
        for u in all_users:
            if u.get("uid") == uid: continue
            users.append({**u, "unread_count": 0, "last_msg": u.get("bio",""), "last_time": 0})
        return jsonify({"users": users})
    except Exception as e:
        print("api_users error:", e, file=sys.stderr)
...
def api_ping():
    ...
    return jsonify({"ok": True, "time": int(__import__('time').time())})

@app.route("/api/test_db")
def api_test_db():
    """Test DB speed step by step"""
    import time as _time
    results = {}
    t0 = _time.time()
    
    # Test 1: basic users query
    try:
        users = supa_get("users", "select=uid,username&limit=5")
        results["users_query"] = f"OK - {len(users)} rows in {(_time.time()-t0)*1000:.0f}ms"
    except Exception as e:
        results["users_query"] = f"ERROR: {e}"
    
    t1 = _time.time()
    # Test 2: messages query
    try:
        msgs = supa_get("messages", "select=id&limit=5")
        results["messages_query"] = f"OK - {len(msgs)} rows in {(_time.time()-t1)*1000:.0f}ms"
    except Exception as e:
        results["messages_query"] = f"ERROR: {e}"
    
    results["total_ms"] = f"{(_time.time()-t0)*1000:.0f}ms"
    results["supabase_url"] = SUPABASE_URL[:30]+"..." if SUPABASE_URL else "KOSONG"
