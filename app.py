...
def api_users():
    ...
    try:
        ...
        if not isinstance(all_users, list):
            return jsonify({"users":[], "error":"Gagal mengambil data users"})

        # Fetch ALL messages involving this user in one query
        all_msgs = []
        try:
            all_msgs = supa_get("messages",
                f"or=(from_uid.eq.{uid},to_uid.eq.{uid})&deleted=eq.false&order=created_at.desc&limit=500&select=chat_id,from_uid,to_uid,message,file_url,created_at,status")
            if not isinstance(all_msgs, list):
                all_msgs = []
        except: pass

        # Build per-chat last_msg and unread from memory
        chat_last = {}   # chat_id -> {msg, time}
        chat_unread = {} # chat_id -> count
        for m in all_msgs:
            cid = m.get("chat_id","")
            if not cid: continue
            if cid not in chat_last:
                txt = m.get("message","") or ("[Media]" if m.get("file_url") else "")
                chat_last[cid] = {"msg": txt, "time": m.get("created_at", 0)}
            if m.get("to_uid") == uid and m.get("status") != "read":
                chat_unread[cid] = chat_unread.get(cid, 0) + 1

        users = []
        for u in all_users:
            if u.get("uid") == uid: continue
            chat_id = "_".join(sorted([uid, u["uid"]]))
            cl = chat_last.get(chat_id, {})
            users.append({
                **u,
                "unread_count": chat_unread.get(chat_id, 0),
                "last_msg": cl.get("msg", ""),
                "last_time": cl.get("time", 0)
            })
