...
<div class="main" id="main-panel">
  ...
  <div id="chat-wrap" style="display:none;flex-direction:column;height:100%;overflow:hidden;">
    ...
    <div class="chat-header" id="chat-header">
      ...
      <div onclick="showContactInfo()" style="flex:1;min-width:0;overflow:hidden;cursor:pointer;padding:0 4px;">
...
<div class="call-ui" id="call-ui">
  <!-- Video grid (shown for video calls) -->
  ...
  <!-- Audio info (shown for audio calls) -->
...
def register():
    ...
    if not result:
        return jsonify({"ok":False,"msg":"Registrasi gagal, coba lagi"})
...
def api_ai_chat():
    ...
    system = data.get("system","Kamu adalah WaClone AI. Jawab dalam Bahasa Indonesia yang ramah.")
    ...
    if not fixed: return jsonify({"ok":False,"msg":"Tidak ada pesan valid"})
    api_key = get_anthropic_key()
    if not api_key: return jsonify({"ok":False,"msg":"API key tidak dikonfigurasi."})
    try:
        payload = json.dumps({"model":"claude-haiku-4-5-20251001","max_tokens":1024,"system":system,"messages":fixed}).encode()
...
def api_setup_info():
    """Returns the SQL to run in Supabase to create all tables"""
    sql = """
