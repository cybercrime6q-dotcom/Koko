...
/* CHAT HEADER - FIXED: tombol call selalu tampil */
...
.header-btn{width:38px;height:38px;border-radius:50%;background:transparent;color:var(--tx);display:flex !important;align-items:center;justify-content:center;transition:.2s;flex-shrink:0;border:none;cursor:pointer;visibility:visible !important;}
.header-btn:hover{background:var(--hv);}
/* FIX: call buttons explicitly forced visible */
.call-btn-audio{display:flex !important;background:rgba(0,168,132,.12);color:var(--g);visibility:visible !important;}
.call-btn-audio:hover{background:rgba(0,168,132,.25);}
.call-btn-video{display:flex !important;background:rgba(83,189,235,.12);color:var(--bl);visibility:visible !important;}
...
/* MOBILE - FIX: tombol call tetap tampil di mobile */
@media(max-width:700px){
  ...
  /* FIX: tombol call tidak boleh hilang di mobile */
  .call-btn-audio{display:flex!important;visibility:visible!important;}
  .call-btn-video{display:flex!important;visibility:visible!important;}
  .chat-header{gap:2px;padding:0 4px;}
}
</style>
</head>
<body>
...
<div class="sb" id="sidebar">
  ...
  <div class="sbtabs">
    ...
    <div class="stab" onclick="switchTab('status')" id="tab-status">ðŸŸ¢ Status</div>
    <div class="stab" onclick="switchTab('live')" id="tab-live">ðŸ”´ Live</div>
...
<div class="main" id="main-panel">
  <div class="no-chat" id="no-chat" style="display:flex;">
    ...
    <div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap;justify-content:center;">
      ...
      <button onclick="switchTab('live')" style="padding:10px 20px;background:linear-gradient(135deg,#e53e3e,#c53030);color:#fff;border:none;border-radius:20px;font-size:14px;font-weight:800;cursor:pointer;font-family:'Nunito',sans-serif;">ðŸ”´ Live</button>
    </div>
  </div>
  <div id="chat-wrap" style="display:none;flex-direction:column;height:100%;overflow:hidden;">
    <!-- CHAT HEADER: nama div dibatasi agar tidak dorong tombol call -->
    <div class="chat-header" id="chat-header">
      ...
      <!-- FIX: max-width lebih kecil supaya tombol call tidak terdorong keluar -->
      <div onclick="showContactInfo()" style="flex:1;min-width:0;max-width:calc(100% - 220px);overflow:hidden;cursor:pointer;padding:0 4px;">
      ...
      <!-- Tombol call dipisah dengan flex-shrink:0 dan min-width agar tidak terpotong -->
      <div style="display:flex;align-items:center;gap:4px;flex-shrink:0;min-width:128px;justify-content:flex-end;">
    ...
    <div class="input-area">
      ...
      <div class="input-row">
        ...
        <button class="side-btn" onclick="toggleEmojiPicker()">ðŸ˜Š</button>
...
<div class="overlay" id="settings-ov">
  <div class="panel" style="width:440px;">
    ...
    <div class="panel-body">
      <div style="margin-bottom:18px;"><div style="font-size:11px;font-weight:800;color:var(--g);text-transform:uppercase;letter-spacing:.6px;margin-bottom:10px;padding-bottom:5px;border-bottom:1px solid var(--bd);">ðŸ’¬ Chat</div>
      ...
      <div><div style="font-size:11px;font-weight:800;color:var(--g);text-transform:uppercase;letter-spacing:.6px;margin-bottom:10px;padding-bottom:5px;border-bottom:1px solid var(--bd);">ðŸ—‘ï¸ Data</div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;">
          ...
          <span style="font-size:12px;color:var(--g);font-weight:700;">v4.1 â€¢ Cloudinary + Supabase</span>
        </div>
      </div>
    </div>
  </div>
    ...
    try:
        ...
        for u in all_users:
            ...
            try:
                ...
                if lm:
                    last_msg = lm[0].get("message","") or ("ðŸ“Ž Media" if lm[0].get("file_url") else "")
...
def api_send_file():
    ...
    try:
        ...
        supa_post("notifications", {
            "to_uid": to_uid, "from_uid": user["uid"],
            "message": "ðŸ“Ž Media", "is_read": False, "created_at": ts
        })
